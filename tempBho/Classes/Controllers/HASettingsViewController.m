...
- (void)removeAdsAction:(UISwitch *)sender
{
    if ([HASettings sharedManager]._isInAppPurchaseSupported == NO) {
    ...
    if (sender.isOn)
    {
        ...
        if (adsRemoved) {
            ...
            {
                if ([[RageIAPHelper sharedInstance] hasProduct:[HASettings sharedManager]._removeAdsProdcutIdentifier])
                {
//                    self._restoreAlertView = [[UIAlertView alloc] initWithTitle:@"" message:@"Purchase remove ads for %@ or choose Restore if you have already purchased" delegate:self cancelButtonTitle:@"Purchase" otherButtonTitles:@"Restore",nil];
//                    self._restoreAlertView.delegate = self;
//                    [self._restoreAlertView show];
                }
                else
                {
                }
    ...
    {
        [[HASettings sharedManager] setAdsTurnedOff:NO];
    }
}
...
{
    if ([HASettings sharedManager]._isAdSupported) {
        return 3;
    }
    return 2;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([HASettings sharedManager]._isAdSupported) {
        if (indexPath.row == 0) {
            ...
            [self._removeAdsSwitch removeFromSuperview];
            self._removeAdsSwitch = [[UISwitch alloc] initWithFrame:CGRectMake(CGRectGetWidth(cell.frame) - 51.0, CGRectGetMidX(cell.frame) - 15.5, 50.0, 31.0)];
            [self._removeAdsSwitch addTarget:self action:@selector(removeAdsAction:) forControlEvents:UIControlEventValueChanged];
            cell.accessoryView = self._removeAdsSwitch;
        }
        ...
        [self._removeAdsSwitch setOn:[HASettings sharedManager]._isAdsTurnedOff];
        [self._soundsSwitch setOn:[HASettings sharedManager]._isSoundsOn];
    }
    else{
        if (indexPath.row == 0) {
        ...
        self._removeAdsSwitch.onTintColor = [HASettings sharedManager]._appTextColor;
...
- (void)productPurchased:(NSNotification *)notification {
    ...
    {
        [[HASettings sharedManager] setAdsTurnedOff:YES];
        [self._removeAdsSwitch setOn:YES];
    }
}
...
- (void)productsLoaded:(NSNotification *)nc
{
    ...
    for (SKProduct * product in [RageIAPHelper sharedInstance]._products)
    {
        if ([product.productIdentifier isEqualToString:productIdentifier])
        {
            self._restoreAlertView = [[UIAlertView alloc] initWithTitle:@"Turn off ads" message:[NSString stringWithFormat:@"Purchase remove ads for %@ or choose Restore if you have already purchased",[[RageIAPHelper sharedInstance] productPriceForProductIdentifier:productIdentifier]] delegate:self cancelButtonTitle:@"Cancel" otherButtonTitles:@"Purchase",@"Restore",nil];
            self._restoreAlertView.delegate = self;
            [self._restoreAlertView show];
        }
    }
